...
if not st.session_state.setup_complete:
    ...
    st.session_state["department"] = st.selectbox(
        "Choose a Department",
       ("Clinical", "Pharmacy", "Laboratory", "Obstretrics & Gynecology", "Radiology", "Medical Records", "Emergency")
    )
    

    # Test labels for company and position information
    st.write(f"**Your information**: {st.session_state['level']} {st.session_state['position']} at {st.session_state['department']}")
...
if st.session_state.setup_complete:
    ...
    if len(st.session_state.messages) == 0:
        st.session_state.messages.append({
            "role": "system",
            "content": (
                f"You are a Hospital HR executive that interviews {st.session_state['name']} "
                f"with experience {st.session_state['experience']} and skills {st.session_state['skills']}. "
                f"You should interview him for the position {st.session_state['level']} "
                f"{st.session_state['position']} at the department {st.session_state['department']}"
            )
        })
...
response_id = len(st.session_state.get("messages", []))
...
with col1:
    if st.button("👍", key=f"up_{response_id}"):
        feedback_value = "up"

with col2:
    if st.button("👎", key=f"down_{response_id}"):
        feedback_value = "down"
